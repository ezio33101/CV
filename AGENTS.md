...
- `styles.js`：轻量脚本，用于自动更新页脚年份、移动端导航开合、中英文界面切换、首页与横向扩展板块切换、联系方式定位与导航高亮。
...
- 页面右上角提供中英文切换按钮；首次访问默认显示英文，语言状态保存在 `localStorage`，之后优先沿用访客已保存的语言选择。新增或修改可见文案时，应同步维护 `styles.js` 中的 `translations` 映射，确保英文界面尽量全英文、中文界面尽量全中文。
...
- 已在首页和三个项目类型页右上角新增中英文切换按钮，并通过 `styles.js` 统一维护双语文案与语言偏好保存。
- 网站首次访问的默认语言已由中文调整为英文；用户主动切换后的语言偏好仍通过 `localStorage` 保留。
